...
Процедура ОбкаткаПолученияСовпадений()
	...
	ТелефоннаяКнига = Новый Структура("Путь", "user/app/music");

	Поиск.ЗагрузитьДанные(ТелефоннаяКнига, "Фамилия,Имя,Отчество,ид");	
	Поиск.НайтиЗначенияИлиКлючи("ueric");
...
Функция РазбитьСтрокуПоПозициям(Знач Текст, Позиции)
	...
	Для каждого ТекПозиция Из Сортировка Цикл
		...
		ИначеЕсли Позиция = 1 или Позиция = СтрДлина(Текст)Тогда
			МассивФразы.Добавить(Новый Структура("Текст, Цвет", Сред(Текст, Позиция, 1), ЦветКонсоли.Красный));
		Иначе
...
Процедура Тесты()
	...
	рез = Поиск.НайтиЗначенияИлиКлючи("amuser", 1, Ложь);

	пр = "app/models/user.rb" = рез;
