...
Процедура ПриСозданииОбъекта(_Поиск, _ТекстПодсказки = "")
	...
	РежимВвода = Истина;
	Поиск = _Поиск;
	ТекстПодсказки = _ТекстПодсказки;
...
Процедура ОтрисоватьЭкран() Экспорт
	...
	Если ЗначениеЗаполнено(ТекстПодсказки) Тогда
		НижняяСтрока = НижняяСтрока - 1;
		ВысотаСписка = ВысотаСписка - 1;
		ВывестиТекстПоКоординатам(ТекстПодсказки, НижняяСтрока, 1);
	КонецЕсли;
