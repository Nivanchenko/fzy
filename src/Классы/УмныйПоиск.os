...
Перем ПослденийРезультат Экспорт;
...
Функция НоваяТаблицаРезультата()
	...
	Для Каждого ИмяКолонки из НовыйМассив("Текст, ТекстАнгл, Вес, ТипЯзыка, Ключ", Ложь) Цикл
...
Функция ВыполнитьПоиск(СтрокаПоиска) Экспорт
	...
	ПослденийРезультат = НоваяТаблицаРезультата();
	Для Каждого Набор из ПодготовленныеДанные Цикл
		...
		ТекущийРезультат = Новый Структура("Текст, Текст, Вес, ТипЯзыка", 
						Набор.Русский.ОригинальныйТекст,
						Набор.Английский.ОригинальныйТекст,
						Вес,
						ТипЯзыка);
		НоваяСтрокаРезультата = ПослденийРезультат.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаРезультата, ТекущийРезультат);

	КонецЦикла;

	ПослденийРезультат.Сортировать("Вес");

	Возврат ПослденийРезультат;
